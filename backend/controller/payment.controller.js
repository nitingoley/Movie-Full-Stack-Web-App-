...
export const createCheckoutSession = async (req, res) => {
    try {
        ...
        if (!process.env.STRIPE_PRICE_ID) {
            return res.status(400).json({ error: "Stripe price ID not configured in environment variables" });
        }
...
export const cancelSubscription = async (req, res) => {
    try {
        ...
        if (!user || !user.subscriptionId) {
            return res.status(400).json({ error: "No active subscription found for the user" });
        }

        console.log(`Canceling subscription for user: ${req.user.id} with subscription ID: ${user.subscriptionId}`);
...
export const updateSubscription = async (req, res) => {
    try {
        ...
        const updatedSubscription = await stripeInstance.subscriptions.update(user.subscriptionId, {
        ...
        res.status(500).json({ error: "Subscription update failed", details: error.message });
    }
};
...
export const getSubscription = async (req, res) => {
    try {
        ...
        const subscriptionStatus = user.subscriptionStatus || "No active subscription";
